...
public class IgnitedHttpRequestRetryHandler implements HttpRequestRetryHandler {
	...
	//change from 1500 to  3000 means 3 seconds
    ...
    public boolean retryRequest(IOException exception, int executionCount, HttpContext context) {
        ...
        if (retry) {
            Log.e(IgnitedHttpClient.LOG_TAG, "request failed (" + exception.getClass().getCanonicalName()
                    + ": " + exception.getMessage() + " / attempt " + executionCount
                    + "), will retry in " + RETRY_SLEEP_TIME_MILLIS / 1000.0 + " seconds");
            SystemClock.sleep(RETRY_SLEEP_TIME_MILLIS);
        } else {
            Log.e(IgnitedHttpClient.LOG_TAG, "request failed after " + executionCount + " attempts");
